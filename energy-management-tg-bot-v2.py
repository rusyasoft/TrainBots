...
def help(bot, update):
    help_msg = u"/start <비밀번호> : 민원 수신 모드로 변경\n"
    help_msg += u"/server_status : 서버 상태 정보 확인\n"
...
def on_message_systemstatus(client, userdata, msg):
    try:
...
def on_message(client, userdata, msg):
    #print 'topic:',msg.topic,' on_message --> ', msg.payload
    pass

def on_message_minwon(client, userdata, msg):
    try:
        #print '-->', msg.payload
        global g_update
        if g_update != None:
...
mqtt_client.message_callback_add("/keti/energy/minwon/#", on_message_minwon)
mqtt_client.message_callback_add("/keti/energy/systemstatus/#", on_message_systemstatus)
...
res = mqtt_client.subscribe("/keti/energy/#", 1)
